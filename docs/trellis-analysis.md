...
steer before the boundary is ever hit). A `<drive-state>` block derived from `$RUN_DIR/state.json`
...
- **Landing surface:** `.claude/commands/drive-retro.md`, invoked at run-wrap alongside the standing /decant; no drive.md edit required for v1.
...
- **String-pin-test exposure:** v1 adds a new file only, touching no pinned artifact; if any `drive*.md` wiring is added during implement, `.claude/commands/drive*.md` is string-pin-tested — run `python3 -m pytest tests/contracts` locally during implement.
