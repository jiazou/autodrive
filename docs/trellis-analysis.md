...
- **Landing surface:** `.claude/commands/drive-retro.md`. v1 is a MANUAL, operator-invoked post-run command — `/drive-retro <runId>` on a completed run, the same way /decant is operator-invoked today — so no drive.md edit is required for v1. Automatic run-wrap wiring (a drive.md Completion-step edit) is a named follow-on, explicitly not part of the spike.
