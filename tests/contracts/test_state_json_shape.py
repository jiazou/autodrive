...
THIS worktree (re-verified for the per-leg-goal / Stop-hook rework):
  ...
  - CLAUDE.md enumerates (top-level): runId, baseRef, featureBranch, phase,
  ...
  - Intersection = the 8 keys below. `stage` is in drive.md but NOT in CLAUDE.md,
    so it is deliberately excluded from CORE (asserted explicitly below).
"""
import json
import re
...
def test_stage_is_documented_only_in_drive_md():
    """Pin the real D9/D11 asymmetry: `stage` lives in drive.md's JSON but NOT in
    CLAUDE.md's description — so it must stay out of CORE_KEYS. This would FAIL if a
    future edit either drops `stage` from drive.md OR starts documenting it in
    CLAUDE.md (i.e. it tracks the real contract, not a tautology)."""
    _, obj = _drive_md_json()
    claude_names = _claude_md_state_keys()
    assert "stage" in obj, "expected drive.md example to carry a top-level `stage`"
    assert "stage" not in claude_names, (
        "CLAUDE.md now documents `stage` — the soft-contract asymmetry changed; "
        "reconsider whether `stage` should join CORE_KEYS"
    )
    assert "stage" not in CORE_KEYS
